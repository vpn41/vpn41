...
def f():
    class F:
        sut = SetupProcessor(app_dir=APP_DIR,
...
def funcs_mock(f):
    ...
    with patch.object(f.sut, '_scratch_setup') as scratch_setup_mock, \
        patch.object(f.sut, '_server_setup') as server_setup_mock, \
        patch.object(f.sut, '_client_setup') as client_setup_mock, \
        patch.object(f.sut, '_read_keys_file') as read_keys_file_mock:
...
@pytest.mark.asyncio
async def test_run_must_handle_exceptions_properly(f):
    with patch.object(f.sut, '_scratch_setup') as scratch_setup_mock:

        scratch_setup_mock.side_effect = RuntimeError

        assert not f.sut.is_completed()
        assert not f.sut.is_ok()

        with pytest.raises(RuntimeError):
            await f.sut.run()

        assert f.sut.is_completed()
        assert not f.sut.is_ok()


@pytest.mark.asyncio
async def test_run_in_first_time_setup_calls_scratch_server_client_setups(f, funcs_mock):
    ...
    assert not f.sut.is_completed()
    assert not f.sut.is_ok()
    assert f.sut.keys_file_content() == None

    assert await f.sut.run() == [SETUP_FUNC_RV, SETUP_FUNC_RV, SETUP_FUNC_RV]

    assert f.sut.keys_file_content() == data
    assert f.sut.is_completed()
    assert f.sut.is_ok()
...
@pytest.mark.asyncio
async def test_run_must_raise_subprocess_error_if_subprocess_failed(f, funcs_mock):
    scratch_setup_mock = funcs_mock.scratch_setup_mock
    server_setup_mock = funcs_mock.server_setup_mock
    client_setup_mock = funcs_mock.client_setup_mock
    read_keys_file_mock = funcs_mock.read_keys_file_mock
...
@pytest.mark.asyncio
async def test_run_in_not_first_time_setup_calls_only_client_setup(f_not_first_time):
    f = f_not_first_time
    data = 'file data'
    with patch.object(f.sut, '_scratch_setup') as scratch_setup_mock, \
         patch.object(f.sut, '_server_setup') as server_setup_mock, \
...
@pytest.mark.asyncio
async def test_scratch_setup_must_call_create_subprocess_shell_properly(f, create_subprocess_shell_mock):
    assert await f.sut._scratch_setup() == SETUP_FUNC_RV
...
@pytest.mark.asyncio
async def test_server_setup_must_call_create_subprocess_shell_properly(f, create_subprocess_shell_mock):
    assert await f.sut._server_setup() == SETUP_FUNC_RV
    create_subprocess_shell_mock.assert_awaited_with(
...
@pytest.mark.asyncio
async def test_client_setup_must_call_create_subprocess_shell_properly(f, create_subprocess_shell_mock):
    assert await f.sut._client_setup() == SETUP_FUNC_RV
    create_subprocess_shell_mock.assert_awaited_with(
